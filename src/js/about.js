// YolongCMS Desktop — 关于我们管理（单语：中文）
(function () {
    'use strict';

    const LANG_FILE = 'zh_about.yml';
    ...
    async function load() {
        ...
        try {
            ...
            const path = repoPath + '/_data/' + LANG_FILE;
            const result = await window.yolongcms.yml.read(path);
            if (result.success && result.data) { populateForm(result.data); showToast('✅ 已加载 ' + LANG_FILE); }
            else { populateForm(getDefaultData()); showToast('ℹ️ 未找到 ' + LANG_FILE + '，已使用默认内容'); }
    ...
    async function save() {
        ...
        try {
            const repoPath = await window.yolongcms.sites.repoPath(site.id);
            await window.yolongcms.yml.write(repoPath + '/_data/' + LANG_FILE, collectForm());
